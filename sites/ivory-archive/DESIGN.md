...
- 日期、期号、关键细节、原文链接与图片说明共同制造档案感。
- 卡片只是首页索引的一部分；长文页回到连续的编辑版式，不把每段都装进卡片。

## Color system

动态应用 `sites/ivory-archive/globals.css` 当前主要变量：
...
- 详情页先用三分钟摘要建立方向，再以编号和四段阅读流组织长文。
- 桌面与移动端都保持“背景 / 事件 / 细节 / 分析”的固定顺序。
- 标题区域不显示来源链接；每篇只在分析后保留一个描述性原文入口。
- 分析与前三段使用相同的连续纸面结构，不单独使用红色底框；原文入口位于分析段落外，以普通文字链接呈现，不与分析组成卡片。
- 细节固定使用项目符号列表，每项末尾有完整标点，并允许因研究设计与发现需要而增加条目。
- 日刊档案使用连续列表，不改成重复卡片墙。
...
- 正文阅读下限为桌面 19px、移动 18px；来源与元信息可以更小，但不能承担唯一解释。
