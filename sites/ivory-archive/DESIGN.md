...
- Primary stylesheet: `sites/ivory-archive/globals.css`
...
Data / Analytics 是档案自我说明的一部分，不是后台仪表盘：

- 继续使用象牙纸、酒红发丝线、森林绿链接和编辑网格。
- Provider、Signals、Privacy 使用档案字段式定义列表，不使用 SaaS 指标卡。
- 无真实数据时只显示测量边界和供应商状态，不伪造访问量、增长率或 KPI。
- 移动端改为单列，但保留同一信息顺序与清晰的 manifest 入口。
- 该区块是本次迁移唯一有意增加的可见结构。
