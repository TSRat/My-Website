...
- `HYPATIA/` is now a generated deploy mirror; edit the site package and run
  `npm run build:hypatia`.
- Historical unreferenced assets remain preserved. No visual, content, URL,
  analytics-provider, or framework change was intended.
...
- 可维护完整页面位于 `sites/hypatia/index.html`；`HYPATIA/` 是生成的发布镜像。
...
- `sites/hypatia/index.html`：内容和结构
- `sites/hypatia/assets/hypatia-site.css`：基础 reset/style
- `sites/hypatia/assets/hypatia-v2.css`：当前主视觉
- `sites/hypatia/assets/hypatia-refresh.js`：当前交互
- `sites/hypatia/assets/agora-hypatia-teaching.jpg`：现代 AI 辅助教学场景视觉隐喻
- `sites/hypatia/assets/`：图片和历史产物
- `sites/hypatia/site.config.json`：统一构建、公开路径和镜像契约
